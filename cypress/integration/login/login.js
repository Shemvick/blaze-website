...
Given ('I navigate to demoblaze ecommerce page', function() {
  ...
  // Visit the website
  cy.navigateToPage(Cypress.env('baseURL')); //Custom code
...
cy.clickOnElement('#login2', 'Log in');//Custom code
...
When('I enter an invalid username and a valid password', function (){
  cy.login(this.data.invalidUsername, this.data.Password)
  ...
  When('I enter a valid username and an invalid password', function(){
    cy.login(this.data.Username, this.data.invalidPassword)
      // cy.get('#loginusername').type(this.data.invalidUsername)
...
When('I click the Log in button', function() {
  ...
  cy.clickOnElement('.btn.btn-primary', 'Log in')
  // cy.get('.btn.btn-primary').contains('Log in').click()
})

Then('I should see the homepage after login', function() {
  // Assert that the user is logged in by checking the visibility of the home page
  cy.get('a').contains('Home').should('be.visible') // Home button visible
  cy.wait(4000)
    cy.get('#nameofuser').get('a').should('be.visible').and('contain', 'Welcome Taiwo');
  // cy.wait(4000)
